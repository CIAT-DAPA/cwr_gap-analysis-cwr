...
class TaxonMySQL implements TaxonDAO {
    ...
    public function suggestTaxaList($id, $includeCommonName, $limit, $term) {
        global $db;
        $query = "SELECT DISTINCT s2.Common_Name, s2.Family, s2.Genus, s2.Species, s2.Subsp, s2.Var, s2.Form, s2.Scientific_Name, s2.Valid_Taxon_ID
        ...
        $query .= "        AND s.Main_Crop = " . $id . " AND LOWER(s.Scientific_Name) LIKE '" . strtolower(strip_tags($term)) . "%'";
        /*
        if ($id == 1) {
            $query .= " AND s2.Valid_Taxon_ID = s2.Taxon_ID";
        }*/
        if ($includeCommonName) {
            $query .= "     OR s2.Common_Name LIKE '%" . strip_tags($term) . "%' ";
            $query .= "         AND s.Main_Crop = " . $id . " AND s2.Valid_Taxon_ID = s2.Taxon_ID";
        }
        $query .= "     OR s2.Family = '" . strip_tags($term) . "' ";
        $query .= "         AND s.Main_Crop = " . $id . " AND s2.Valid_Taxon_ID = s2.Taxon_ID";
        $query .= "         OR LOWER(s2.Scientific_Name) = '" . strtolower(strip_tags($term)) . "'
                            AND s.Main_Crop = " . $id . " AND s2.Valid_Taxon_ID = s2.Taxon_ID AND LOWER(s.Scientific_Name) = '" . strtolower(strip_tags($term)) . "'
                            OR s2.Common_Name = '" . strip_tags($term) . "'
                            AND s.Main_Crop = " . $id . " AND s2.Valid_Taxon_ID = s2.Taxon_ID 
