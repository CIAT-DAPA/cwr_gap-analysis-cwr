...
extractClimates <- function(input_dir,sample_file,env_dir,env_prefix,
  ...
  for (i in 1:19) {
    ...
    rs <- raster(paste(envdir,"/",env_prefix,env_ext,sep=""))
    swd$NEW <- extract(rs,xy)
    names(swd)[i+3] <- paste(prefix,i,sep="")
  }
